...
int parse_deftype (module_t* mod, type_t* type, node_t* node) {
  switch (node->list->fst->next->type) {
    case NODE_ATOM:
      if (parse_type(mod, node->list->fst->next->next, type) != 0) {
        return 1;
      }
      break;
    case NODE_LIST:
      if (parse_type_template(mod, node->list->fst->next, type) != 0) {
        return 1;
      }
      break;
  }
  ...
  for (int i = 0; i < type->num_fields; i++) {
    ...
    switch (node->type) {
      ...
      case NODE_LIST:
        ...
        switch (parse_or_find_type(mod, node, &field)) {
          case 0:
          ...
          case 1:
            break;
          case 2:
            // supports anonymous sum types in fields
            field->name = type->field_names[i];
            break;
        }
        break;
    }
